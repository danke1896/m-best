...
% Detection parameter
Prun_Thre=param.Prun_Thre;% Parameter for pruning detections with the confidence score less than this value检测到的置信度小于预知的就抛弃

% Track termination parameter
Term_Frame=param.Term_Frame;% The parameter for termination condition终止的条件
...
mbest=param.N_H; % Threshold for considering m-best number of Hypotheses for JPDA不能超过100个目标

% IMM model (Constant or Sate-Dependent) ？？？这个到底是什么？
...
%Initial State Vector & IMM Initial Parameters第一帧时的目标状态信息和传输的协方差信息
Xe=cell(Kt,N_Target);
XeT=cell(1,N_Target);
Pe=cell(Kt,N_Target);
PeT=cell(1,N_Target);
...
if strcmp(Plott,'Yes')
    ...
    if strcmp(Box_plot,'Yes')
        for no=1:N_Target
            ...
            Hh=H_D2(no);
            hold on
            rectangle('Position',[Xx,Yy,Ww,Hh],'EdgeColor',colorord(no,:),'LineWidth',5)
...
for f=2:Frame
    ...
    N_Target=size(Xe,2);
    Mes_Tar=false(size(XYZ{1,f},1),N_Target,Kt);%表示量测值域目标之间的逻辑关系，行为量测值个数，列为目标个数
    ...
    Target_probabilty=cell(N_Target,Kt);%这个p是指的那个概率密度q()or P()；
    ...
    for no=1:N_Target
        if ~ismember(no,Terminated_objects_index)
            k=f-Ff{1,no}(1,1)+1;%Ff指的是什么，状态转移概率？k指的是什么？
            C{1,no}(:,k-1)=mui{1,no}(k-1,:)*PT{1,no};%联合概率最小化的问题即文章中的C*1
            X0j=zeros(DSV,Kt);
            ...
            for r2=1:Kt
                ...
                for r1=1:Kt
                    ...
                    X0j(:,r2)=X0j(:,r2)+muij{1,no}(r1,r2,k-1)*Xe{r1,no}(:,k-1);%Image均值
                end
                
                for r1=1:Kt
                    PI0j(:,:,r2)=PI0j(:,:,r2)+muij{1,no}(r1,r2,k-1)...
                        *(Pe{r1,no}(:,:,k-1)+(Xe{r1,no}(:,k-1)-X0j(:,r2))...
                        *(Xe{r1,no}(:,k-1)-X0j(:,r2))');%均方差
                end
                ...
                Lambda{1,no}(r2,k)=sum(Target_probabilty{no,r2}{1}{1});%lambda是用来干什么的？
    ...
    if ~isempty(New_Targets)
        for ij=1:length(New_Targets)
            ...
            if strcmp(Plott,'Yes')&&strcmp(Box_plot,'Yes')
                ...
                hold on
                rectangle('Position',[Xx,Yy,Ww,Hh],'EdgeColor',colorord(N_Target+ij,:),'LineWidth',5)
