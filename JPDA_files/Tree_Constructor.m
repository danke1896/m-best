...
TMindx=cell(1,msp);%目标域量测值的索引号
TMprob=cell(1,msp);%量测是由目标生成的概率
X_u=[];
ssD=zeros(1,msp);
...
for i=1:msp
    ...
    for j=1:SizD
        Sigma_e=Q+F*Sigma_p(:,:,j)*F';%目标状态协方差
        S=H*Sigma_e*H'+R;%观测协方差
        K=Sigma_e*H'/S;%比值
        if i==1
            X_E=X_e;
            Sigma_E=Sigma_e;
        ...
        if isempty(TMindx{i}{j})
            ...
            TMprob{i}{j}=[(1-pD)*Beta;gij*pD];%第i个量测值是由第J个目标产生的概率就是文章中的Pt(dji)
        end
        if i==1
